...
Future<void> main() async {
  ...
  teledart.onCommand('geo').listen((message) async{
    teledart.sendMessage(583665582, 'zaishlo'); //замість 000 має бути id користувача
...
class DeviceScreen extends StatelessWidget {
        ...
        child: Column(
          children: <Widget>[
            ...
            StreamBuilder<int>(
              ...
              initialData: 0,
              builder: (c, snapshot) => ListTile(
                title: const Text('Send L1 command'),
                trailing: IconButton(
                  icon: const Icon(Icons.edit),
                  onPressed: () => sendCommandL1(),
                ),
              ),
            ),
  ...
  void sendCommandL1() async {
    List<BluetoothService> services = await device.discoverServices();

    services.forEach((service) async {
      if (service.uuid.toString() == 'UUID_СЕРВІСУ') {
        List<BluetoothCharacteristic> characteristics = service.characteristics;

        characteristics.forEach((characteristic) async {
          if (characteristic.uuid.toString() == 'UUID_ХАРАКТЕРИСТИКИ') {
            await characteristic.write([0x4C, 0x31]);
          }
        });
      }
    });
